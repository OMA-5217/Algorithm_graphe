...
# Mode d'effacement pour supprimer un sommet ou une arête
def activer_mode_effacer():
    canvas.bind("<Button-1>", effacer_element)
    messagebox.showinfo("Mode Effacer", "Cliquez sur un sommet ou une arête pour l'effacer")

# Fonction pour effacer un sommet ou une arête cliqué
def effacer_element(event):
    global sommet_list, arete_list
    x, y = event.x, event.y
    rayon = 15

    # Vérifier si le clic est sur un sommet
    for i, (sx, sy) in enumerate(sommet_list):
    ...
    # Vérifier si le clic est sur une arête
    for i, ((x1, y1), (x2, y2)) in enumerate(arete_list):
        distance = abs((y2 - y1) * x - (x2 - x1) * y + x2 * y1 - y2 * x1) / math.hypot(x2 - x1, y2 - y1)
        if distance < 5:  # Seuil pour cliquer sur l'arête
            arete_list.pop(i)
            redraw_canvas()
            return

# Fonction pour redessiner le graphe
def redraw_canvas():
    canvas.delete("all")
...
menu_effacer.add_command(label="Effacer un sommet ou une arête", command=activer_mode_effacer)
