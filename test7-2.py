...
def creation_sommet(event):
    ...
    # Si aucune proximité détectée, créer le sommet avec une étiquette unique
    sommet_list.append((x, y, next_label))
    radius = 15
    canvas.create_oval(x - radius, y - radius, x + radius, y + radius, fill="lightblue", outline="black")
    canvas.create_text(x, y, text=str(next_label), font=("Arial", 12))
    
    next_label += 1  # Incrémente l'étiquette pour le prochain sommet
...
def creation_arete(event):
    ...
    for i, (sx, sy, label) in enumerate(sommet_list):
        distance = math.sqrt((sx - x) ** 2 + (sy - y) ** 2)
        if distance < distance_min and distance <= 20:
            distance_min = distance
            sommet_proche = (sx, sy, label)

    if sommet_proche:
        selected_sommets.append(sommet_proche)

    if len(selected_sommets) == 2:
        x1, y1, label1 = selected_sommets[0]
        x2, y2, label2 = selected_sommets[1]
...
def redraw_canvas():
    canvas.delete("all")
    for x, y, label in sommet_list:
        radius = 15
        canvas.create_oval(x - radius, y - radius, x + radius, y + radius, fill="lightblue", outline="black")
        canvas.create_text(x, y, text=str(label), font=("Arial", 12))
